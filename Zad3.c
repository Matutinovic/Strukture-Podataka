...
int main(int argc, char** argv)
{
	...
	while (1) {
		...
		case 'D':
			Datoteka(p->next);
			printf("Lista se nalazi u datoteci 'Osobe.txt'\n\n");
			break;
...
{
	FILE* osobe = NULL;
	osobe = fopen("Osobe.txt", "w");
	if (!osobe) {
		perror("Greska pri otvaranju datoteke!");
		return -1;
	}
	Pozicija temp = p;
	fprintf(osobe, "LISTA OSOBA:\n");
	while (temp != NULL) {
		fprintf(osobe, "%s %s %d\n", temp->ime, temp->prezime, temp->godiste);
		temp = temp->next;
	}
...
int CitajIzDatoteke(Pozicija p)
{
	...
	while (!feof(osobe))
	{
		...
		fgets(buffer, 1024, osobe);
		sscanf(buffer, "%s %s %d", q->ime, q->prezime, &q->godiste);
		q->next = p->next;
		p->next = q;
		p = q;
	}
